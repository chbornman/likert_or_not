...
export default function LikertScale({
  ...
  const getButtonColor = (option: number, isSelected: boolean) => {
    ...
    if (isSelected) {
      ...
      if (position <= 60) return 'bg-yellow-500 hover:bg-yellow-600';
  ...
  return (
    <div className="space-y-3">
      <div className="flex justify-between text-sm text-gray-600 px-2">
        <span className="font-medium text-[#B87B7C]">{minLabel}</span>
        <span className="font-medium text-[#7FB069]">{maxLabel}</span>
      </div>
        ...
        {options.map((option) => (
          <button
            ...
            className={`
              ...
              focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#1E5C8B]
            `}
      ...
      <div className="sm:hidden mt-4">
        <input
          ...
          className="w-full h-2 bg-gradient-to-r from-[#B87B7C] via-[#FFD700] to-[#7FB069] rounded-lg appearance-none cursor-pointer"
          style={{
            background: `linear-gradient(to right, #B87B7C 0%, #FFD700 50%, #7FB069 100%)`,
          }}
        />
