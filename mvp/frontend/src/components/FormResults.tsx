...
export default function FormResults() {
  ...
  const fetchFormData = async () => {
    try {
      ...
      const respondentsRes = await fetch(`/api/admin/forms/${formId}/respondents?token=${token}`);
  ...
  const exportToExcel = () => {
    ...
    const getRoleBasedStats = (questionId: string | number, role?: string) => {
      const roleResponses = role 
        ...
        : filteredResponses;
      return roleResponses.map(r => r.answers[String(questionId)]).filter(Boolean);
    };
    ...
    summaryData.push(['Report Type:', selectedRole !== 'all' ? `Filtered by Role: ${selectedRole}` : 'All Responses']);
    summaryData.push(['']);
    ...
    if (availableRoles.length > 0) {
      ...
      summaryData.push(['-'.repeat(40)]);
      summaryData.push(['Role', 'Count', 'Percentage', 'Completed', 'Completion Rate']);
    ...
    sections.forEach(section => {
      ...
      sectionQuestions.forEach(question => {
        ...
        if (questionType === 'likert') {
          ...
          const values = allResponses.map(a => a.likert_value).filter(v => v != null);
          ...
          summaryData.push(['']);
          summaryData.push(['Distribution:', '', '1 (SD)', '2 (D)', '3 (N)', '4 (A)', '5 (SA)']);
          
          const distribution = [1, 2, 3, 4, 5].map(val => values.filter(v => v === val).length);
          summaryData.push(['Count:', '', ...distribution]);
          summaryData.push(['Percentage:', '', ...distribution.map(d => 
            values.length > 0 ? `${((d / values.length) * 100).toFixed(1)}%` : '0%'
          )]);
          
          // Role-based breakdown for Likert
          if (availableRoles.length > 1) {
            summaryData.push(['']);
            summaryData.push(['By Role:', 'Responses', 'Average', '1', '2', '3', '4', '5']);
            availableRoles.forEach(role => {
              const roleAnswers = getRoleBasedStats(question.id, role);
              const roleValues = roleAnswers.map(a => a.likert_value).filter(v => v != null);
              if (roleValues.length > 0) {
                const roleAvg = (roleValues.reduce((sum, v) => sum + v, 0) / roleValues.length).toFixed(2);
                const roleDist = [1, 2, 3, 4, 5].map(val => roleValues.filter(v => v === val).length);
                summaryData.push([role, roleValues.length, roleAvg, ...roleDist]);
              }
            });
          }
          ...
          // Rating analysis
          const values = allResponses.map(a => {
          ...
          if (values.length > 0) {
            ...
            for (let i = min; i <= max; i++) {
              ...
              counts.push(count);
              percentages.push(values.length > 0 ? `${((count / values.length) * 100).toFixed(1)}%` : '0%');
            }
          ...
          const values = allResponses.map(a => a.value || a.text_value).filter(Boolean);
          ...
          options.forEach((option: string) => {
            ...
            summaryData.push([
              option,
              count,
              values.length > 0 ? `${((count / values.length) * 100).toFixed(1)}%` : '0%'
            ]);
          });
          ...
          // Yes/No analysis
          const values = allResponses.map(a => a.value || a.text_value).filter(Boolean);
          const yesCount = values.filter(v => v === 'yes' || v === 'Yes' || v === true).length;
          const noCount = values.filter(v => v === 'no' || v === 'No' || v === false).length;
          
          summaryData.push(['Yes:', yesCount, `${values.length > 0 ? ((yesCount / values.length) * 100).toFixed(1) : 0}%`]);
          summaryData.push(['No:', noCount, `${values.length > 0 ? ((noCount / values.length) * 100).toFixed(1) : 0}%`]);
          
        } else if (questionType === 'checkbox') {
          // Checkbox analysis (multiple selections)
          const allSelections = allResponses.map(a => {
            const val = a.value || a.text_value;
          ...
          const options = question.features?.options || [];
          summaryData.push(['Options:', 'Times Selected', 'Percentage of Respondents']);
          options.forEach((option: string) => {
            const count = allSelections.filter(v => v === option).length;
            summaryData.push([
              option,
              count,
              allResponses.length > 0 ? `${((count / allResponses.length) * 100).toFixed(1)}%` : '0%'
            ]);
          });
    ...
    sections.forEach(section => {
      ...
      sectionQuestions.forEach(question => {
        ...
        if (questionComments.length > 0) {
          ...
          sectionComments.push([`Total Comments/Responses: ${questionComments.length}`]);
          sectionComments.push(['']);
          ...
          {showRespondents && (
            <CardContent>
              {respondents.length === 0 ? (
                <p className="text-gray-500 text-center py-4">No respondents yet</p>
              ) : (
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {respondents
                        ...
                        <tr key={respondent.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {new Date(respondent.submitted_at).toLocaleDateString('en-US', {
                              month: 'short',
                              day: 'numeric',
                              year: 'numeric',
                              hour: '2-digit',
                  ...
                  <div className="space-y-4">
                    {sectionQuestions.map(question => {
                      ...
                      if (questionType === 'likert') {
                        ...
                        const averageNum = values.length > 0 
                          ? values.reduce((sum, v) => sum + v, 0) / values.length
                          : 0;
                        const average = values.length > 0 ? averageNum.toFixed(2) : 'N/A';
                        ...
                        const getAverageLabel = (avg: number) => {
                          ...
                          if (avg < 2.25) return avg < 1.75 ? 'Strongly Disagree+' : 'Disagree-';
                          if (avg < 2.75) return avg < 2.5 ? 'Disagree' : 'Disagree+';
                          if (avg < 3.25) return avg < 3 ? 'Neutral-' : 'Neutral';
                          if (avg < 3.75) return avg < 3.5 ? 'Neutral+' : 'Agree-';
                          if (avg < 4.25) return avg < 4 ? 'Agree' : 'Agree+';
                          if (avg < 4.75) return avg < 4.5 ? 'Strongly Agree-' : 'Strongly Agree';
                        ...
                        const distribution = [1, 2, 3, 4, 5].map(val => ({
                          ...
                          percentage: values.length > 0 ? (values.filter(v => v === val).length / values.length * 100) : 0
                        }));
                        
                        const maxCount = Math.max(...distribution.map(d => d.count), 1);

                        return (
                          <div key={question.id} className="border-l-4 border-cambridge-blue/30 pl-2 sm:pl-4 pb-2">
                            ...
                            <div className="flex gap-1 sm:gap-2">
                              {distribution.map(d => {
                                ...
                                const showAbove = barHeight < 20 && d.count > 0;
                                ...
                                return (
                                  <div key={d.value} className="flex-1 text-center">
                                    <div className="relative h-12 sm:h-16 flex items-end">
                                      ...
                                      >
                                        {!showAbove && d.count > 0 && (
                                          <div className="text-xs font-semibold pt-1 text-white">{d.count}</div>
                                        )}
                                      </div>
                                    </div>
                                    <div className="text-xs mt-1 font-medium">{d.value}</div>
                                    <div className="text-xs text-gray-500 hidden sm:block">{d.label}</div>
                                    <div className="text-xs text-gray-500 sm:hidden">
                                      {d.value === 1 ? 'SD' : d.value === 2 ? 'D' : d.value === 3 ? 'N' : d.value === 4 ? 'A' : 'SA'}
                                    </div>
                                  </div>
                                );
                              })}
                        ...
                        const values = questionResponses.map(a => {
                          const val = a.value || a.text_value;
                          return typeof val === 'number' ? val : parseInt(String(val));
                        }).filter(v => !isNaN(v));
                        ...
                        for (let i = min; i <= max; i++) {
                          distribution.push({
                            ...
                            percentage: values.length > 0 ? (values.filter(v => v === i).length / values.length * 100).toFixed(1) : '0'
                          });
                        }

                        return (
                          <div key={question.id} className="border-l-4 border-amber-500/30 pl-2 sm:pl-4 pb-2">
                            <p className="font-medium text-gray-800 mb-2 text-sm sm:text-base">
                              {question.title}
                            </p>
                            <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs sm:text-sm mb-3">
                              <div className="flex items-center gap-1 sm:gap-2">
                                <span className="text-gray-600">Average: </span>
                                <span className="font-bold text-amber-600">{average}</span>
                                {ratingStyle === 'stars' && average !== 'N/A' && (
                                  <div className="flex text-xs sm:text-sm">
                                    {[...Array(max - min + 1)].map((_, i) => (
                                      <span key={i} className={i < Math.round(parseFloat(average) - min + 1) ? 'text-amber-500' : 'text-gray-300'}>
                                        ★
                                      </span>
                                    ))}
                                  </div>
                            ...
                            <div className="space-y-2">
                              {distribution.map(d => (
                                <div key={d.value} className="flex items-center gap-1 sm:gap-2">
                                  <div className="flex-shrink-0 flex items-center text-xs sm:text-sm">
                                    {ratingStyle === 'stars' ? (
                                      <div className="flex">
                                        <span className="text-amber-500">
                                          {[...Array(d.value)].map(() => '★').join('')}
                                        </span>
                                        <span className="text-gray-300">
                                          {[...Array(max - d.value)].map(() => '★').join('')}
                                        </span>
                                      </div>
                                    ) : (
                                      <span className="font-medium w-8 text-center">{d.value}</span>
                                    )}
                                  </div>
                                  <div className="flex-1 bg-gray-200 rounded-full h-5 sm:h-6 relative min-w-0">
                                    ...
                                    >
                                      {d.count > 0 && parseFloat(d.percentage) > 15 && (
                                        <span className="text-xs text-white font-medium">{d.count}</span>
                                      )}
                                    </div>
                                  </div>
                                  <span className="text-xs text-gray-600 w-10 sm:w-12 text-right flex-shrink-0">{d.percentage}%</span>
                                </div>
                              ))}
                            </div>
                          </div>
                        ...
                        const values = questionResponses.map(a => a.value || a.text_value).filter(Boolean);
                        const options = question.features?.options || [];
                        
                        const distribution = options.map((option: string) => ({
                          option,
                          count: values.filter((v: any) => v === option).length,
                          percentage: values.length > 0 ? (values.filter((v: any) => v === option).length / values.length * 100).toFixed(1) : 0
                        }));

                        return (
                          <div key={question.id} className="border-l-4 border-purple-500/30 pl-4 pb-2">
                            ...
                            <div className="text-sm mb-3">
                              <span className="text-gray-600">Responses: </span>
                              <span className="font-bold">{values.length}</span>
                              <span className="text-gray-500 ml-2">({questionType === 'dropdown' ? 'dropdown' : 'multiple choice'})</span>
                            </div>
                            <div className="space-y-2">
                              {distribution.map((d: any) => (
                                <div key={d.option} className="flex items-center gap-2">
                                  <div className="w-32 text-sm truncate" title={d.option}>{d.option}</div>
                                  ...
                                  <span className="text-xs text-gray-600 w-12 text-right">{d.percentage}%</span>
                                </div>
                              ))}
                            </div>
                          </div>
                        ...
                        const allSelections = questionResponses.map(a => {
                          const val = a.value || a.text_value;
                          if (typeof val === 'string') {
                            try {
                              return JSON.parse(val);
                        ...
                        const distribution = options.map((option: string) => ({
                          option,
                          count: allSelections.filter((v: any) => v === option).length,
                          percentage: questionResponses.length > 0 ? (allSelections.filter((v: any) => v === option).length / questionResponses.length * 100).toFixed(1) : 0
                        }));

                        return (
                          <div key={question.id} className="border-l-4 border-indigo-500/30 pl-4 pb-2">
                            <p className="font-medium text-gray-800 mb-2">
                              {question.title}
                            </p>
                            <div className="text-sm mb-3">
                              <span className="text-gray-600">Responses: </span>
                              <span className="font-bold">{questionResponses.length}</span>
                              <span className="text-gray-500 ml-2">(checkbox - multiple selections)</span>
                            </div>
                            <div className="space-y-2">
                              {distribution.map((d: any) => (
                                <div key={d.option} className="flex items-center gap-2">
                                  ...
                                  <div className="flex-1 bg-gray-200 rounded-full h-6 relative">
                                    <div 
                                      ...
                                      style={{ width: `${Math.min(100, parseFloat(d.percentage))}%` }}
                                    >
                                      {d.count > 0 && (
                                        <span className="text-xs text-white font-medium">{d.count}</span>
                                      )}
                        ...
                        return (
                          <div key={question.id} className="border-l-4 border-green-500/30 pl-4 pb-2">
                            <p className="font-medium text-gray-800 mb-2">
                              {question.title}
                            </p>
                            <div className="text-sm mb-3">
                            ...
                            <div className="space-y-2">
                              <div className="flex items-center justify-between text-sm">
                                <div className="flex items-center gap-4">
                                  <span className="font-medium text-green-600">Yes: {yesCount} ({yesPercentage.toFixed(0)}%)</span>
                                  <span className="font-medium text-red-600">No: {noCount} ({noPercentage.toFixed(0)}%)</span>
                                </div>
                              </div>
                                  ...
                                  <>
                                    ...
                                    >
                                      {yesCount > 0 && yesPercentage > 10 && (
                                        <span className="text-xs text-white font-bold">Yes</span>
                                      )}
                                    </div>
                                    <div 
                                    ...
                                    >
                                      {noCount > 0 && noPercentage > 10 && (
                                        <span className="text-xs text-white font-bold">No</span>
                                      )}
                                    </div>
                                  </>
                                )}
                        ...
                        return (
                          <div key={question.id} className="border-l-4 border-teal-500/30 pl-4 pb-2">
                            <p className="font-medium text-gray-800 mb-2">
                              {question.title}
                            </p>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                              ...
                              <div>
                                <span className="text-gray-600">Average: </span>
                                <span className="font-bold text-teal-600">{average}</span>
                              </div>
                              <div>
                      ...
                      } else if (questionType === 'datetime' || questionType === 'date' || questionType === 'time') {
                        const values = questionResponses.map(a => a.value || a.text_value || a.date_value).filter(Boolean);
                        ...
                        return (
                          <div key={question.id} className="border-l-4 border-blue-500/30 pl-4 pb-2">
                            <p className="font-medium text-gray-800 mb-2">
                              {question.title}
                            </p>
                            ...
                            {values.length > 0 && (
                              <details className="text-xs text-gray-600">
                                <summary className="cursor-pointer hover:text-gray-800 font-medium">
                                  View all {values.length} response{values.length !== 1 ? 's' : ''}
                                </summary>
                        ...
                        return (
                          <div key={question.id} className="border-l-4 border-gray-400/30 pl-4 pb-2">
                            <p className="font-medium text-gray-800 mb-2">
                              {question.title}
                            </p>
                            <div className="text-sm mb-2">
                                ...
                                <div className="mt-2 space-y-2 max-h-96 overflow-y-auto">
                                  {values.map((v, i) => (
                                    <div key={i} className="bg-gray-50 px-3 py-2 rounded border border-gray-200">
                                      <div className="text-xs text-gray-500 mb-1">Response {i + 1}</div>
                        ...
                        return (
                          <div key={question.id} className="border-l-4 border-gray-300/30 pl-4 pb-2">
                            <p className="font-medium text-gray-800 mb-2">
                              {question.title}
                            </p>
