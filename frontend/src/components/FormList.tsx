  ...
  useEffect(() => {
    const loadForms = async () => {
      try {
        ...
        const visibleForms = formList.filter(
          (f) => f.status === "published" || f.status === "finished",
        );

        // If there's only one published form (not finished), redirect directly to it
        const publishedOnly = visibleForms.filter(
          ...
          {forms.map((form) => {
            ...
            if (isFinished) {
              return (
                ...
                >
                  <Card
                    className={`shadow-lg ${!isFinished ? "hover:shadow-xl transform hover:-translate-y-1" : ""} transition-all duration-200 border-green-400/30`}
                  >
                    <CardHeader
                      className={`bg-gradient-to-r ${isFinished ? "from-gray-400/10 to-gray-300/10" : "from-cerulean/10 to-cambridge-blue/10"}`}
                    >
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <CardTitle
                            className={`text-xl ${isFinished ? "text-gray-600" : "text-gunmetal group-hover:text-cerulean"} transition-colors`}
                          >
                            {form.title}
                          </CardTitle>
                          {form.description && (
            ...
            return (
              ...
              >
                <Card
                  className={`shadow-lg hover:shadow-xl transform hover:-translate-y-1 transition-all duration-200 border-green-400/30`}
                >
                  <CardHeader
                    className={`bg-gradient-to-r from-cerulean/10 to-cambridge-blue/10`}
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <CardTitle
                          className={`text-xl text-gunmetal group-hover:text-cerulean transition-colors`}
                        >
                          {form.title}
                        </CardTitle>
                        {form.description && (
